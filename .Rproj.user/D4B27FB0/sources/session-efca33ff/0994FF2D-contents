# 当前脚本应该在01_project0789目录下，而不是在分析点目录04_altitude_gout里面
# 清除当前环境所有变量并返回内存使用情况
rm(list = ls()); gc()
...
if (file.exists(file.path(output, "01.elevation_data.csv"))) {
  ...
  for(i in 1:nrow(gbd_countries)) {
    ...
    tryCatch({
      # 获取国家的海拔数据，如果没有有效值，则使用exact_extract计算均值
...
process_gout_indicator <- function(indicator_type) {
  ...
  # 参数说明：
  # indicator_type: 痛风指标类型字符串，可选值为 "asir"、"aspr"、"asdr"
  ...
  # 筛选GBD数据：性别:Both，对应的measure类型，提取国家名和数值
  ...
  significant <- abs(cor_result$estimate) > 0.3 & cor_result$p.value < 0.05 # 定义显著相关标准：|cor| > 0.3 且 p < 0.05
...
# 使用process_gout_indicator函数进行分析
