# 当前脚本应该在01_project0789目录下，而不是在分析点目录06_ksf_ssb里面
# 清除当前环境所有变量并返回内存使用情况
rm(list = ls()); gc()
...
posterior_long_global <- as.data.frame(posterior_preds_global) %>%
  ...
  mutate(
    ...
    intake = exp(log_intake)  # 转换为实际摄入量
  ) %>%
...
# 设置RR矩阵
# 渐进式RR设定避免global级别下某些人口学分组（如高年龄组）PAF计算时产生不合理95%UI
RR_fixed <- exp(meta_result$TE.common)
RR_matrix <- matrix(1, nrow = 1000, ncol = 6)
RR_matrix[, 1] <- 1.0          # 0-25g: 参考组
RR_matrix[, 2] <- 1.2         # 25-50g: 微小风险
RR_matrix[, 3] <- 1.4          # 50-100g: 小风险
RR_matrix[, 4] <- 1.6          # 100-150g: 中等风险
RR_matrix[, 5] <- 1.8         # 150-200g: 较高风险
...
plot_data$category <- factor(plot_data$category,
                             levels = c("Sex", "Education level", "Area of residence",
                                        "Age category (years)"))
# 创建横向柱状图，教育按High->Low，年龄按20-24->95+
...
get_group_asir <- function(group_type, group_name) {
  if(group_name == "Male") {
    ...
    # 教育、居住地用全球Both性别age-standardized
    asir_row <- inv_data %>% 
...
# 给group_label设置正确的factor levels
