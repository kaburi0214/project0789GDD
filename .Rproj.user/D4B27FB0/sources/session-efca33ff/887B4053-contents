# 当前脚本应该在01_project0789目录下，而不是在分析点目录01_burdenanalysis里面
# 清除当前环境所有变量并返回内存使用情况
rm(list = ls()); gc()
...
extract_gbd_data <- function(data, target_age, m_name, target_year, value_prefix) {
  ...
  # m_name: 度量名称
  # target_year: 年份
  # value_prefix: 新列前缀 (如 "n_2021", "asr_2021", "asr_1990")
...
# 计算percentage change 1990-2021
percentage_change <- function(data_2021, data_1990, 
                              correlation = 0.9, n_sim = 1000){ #假设1990年和2021年数据相关系数是0.9
  # 数据匹配和合并
...
# 整合val, upper, lower为val(lower-upper)形式
...
#整合N，2021；ASR，2021; Percentage change 1990-2021(%)信息
...
# 定义7个Super Regions并区分是否存在下级regions
super_regions <- c(
  "High-income",
...
make_summary_table <- function(df) {
  ...
  #location_name: 地区
  #sex_name: 性别
  #n_2021_formatted: 格式化后的N，2021数据
  #asr_2021_formatted: 格式化后的ASR，2021数据
  #change_formatted: 格式化后的Percentage change 1990-2021(%)数据
  incidence <- df %>%
