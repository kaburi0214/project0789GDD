...
country_mapping <- codebook %>%
   dplyr::select(iso3, ...2) %>%  
  rename(location_name = iso3, iso3 = ...2) %>% 
  filter(
    !is.na(location_name), 
    !is.na(iso3),
    location_name != "Label",  
...
attributable_burden_countries <- paf_results_by_country %>%
  ...
  left_join(asir_data, by = "location_name") %>%              
  left_join(aspr_data, by = "location_name") %>%                
  left_join(asdr_data, by = "location_name") %>%              
  filter(!is.na(location_name), !is.na(asir)) %>%           
  mutate(
